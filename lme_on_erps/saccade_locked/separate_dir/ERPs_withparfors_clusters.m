% ERP Analysis Script 
cd \\psyger-stor02.d.uzh.ch\methlab\Neurometric\Anti_newest\THETAproject\martyna\scripts_for_running_tfr_analysis\lme_on_erps\saccade_locked\separate_dir
...
parfor sj = 1:numel(d)
    try
        ...
        anti_erp = cell(1, 3);
        for c = 1:3
            pro_erp{c} = mean(saccEEG.data(clusters{c}, :, pro_indices), 3);
            anti_erp{c} = mean(saccEEG.data(clusters{c}, :, anti_indices), 3);
        end
        
        results{sj} = struct('age_group', age_group, 'pro_erp', {pro_erp}, 'anti_erp', {anti_erp}, 'times', saccEEG.times);
...
for c = 1:3
    ...
    title(['SACCADE LOCKED ERP: Pro vs Anti Saccades (' cluster_names{c} ' Cluster)'], ...
    ...
    line([0 0], [-1.5,  1], 'Color', 'k', 'LineStyle', '--', 'HandleVisibility', 'off');
    ...
    saveas(gcf, fullfile([' SACCADE LOCKED Grand_Average_ERP_pro_vs_anti_' cluster_names{c} '.png']));
end
