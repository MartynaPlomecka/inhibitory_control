...
age_coeff = 'AgeGroup_young';
Mask = Results.(age_coeff).Mask;

age_idx = find(strcmp(lme_model.CoefficientNames, 'AgeGroup_young'));

% betas is a 105x301x6 matrix (channels x time x coefficients)
betas_to_use = betas(:, :, age_idx);
...
sgtitle('Age Coefficient', 'FontSize', 18, 'FontWeight', 'normal', 'FontName', 'Helvetica');
