...
% mass univariate linear regressions
...
% perm considering bin-level MeanRT and shuffling AgeGroup
...
save('stimlock_ERP_lme_TFCE_Results.mat', 'Results', 'betas', 't_obs', 'X');
...
for i = 1:length(unique_electrodes)
    ...
    time_ranges = findContinuousTimeRanges(electrode_times);
...
disp('All time points are relative to stimulus onset (0 ms)');
...
%%
disp('Significant periods between 0 and 300 ms after stimulus onset:');
for i = 1:length(unique_electrodes)
    electrode = unique_electrodes{i};
    time_ranges = electrode_time_ranges{i};
    electrode_number = electrode_map(electrode);

    valid_ranges = time_ranges(time_ranges(:,1) >= 0 & time_ranges(:,2) <= 300, :);
...
function ranges = findContinuousTimeRanges(times)
if isempty(times)
    ranges = [];
    return;
end
