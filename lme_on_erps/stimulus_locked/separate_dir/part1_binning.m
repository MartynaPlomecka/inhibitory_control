...
for sj = 1:numel(d)
    try
        ...
        pro_left_indices = find(strcmp({stimulus_events.cond}, 'pro') & strcmp({stimulus_events.dir}, 'left'));
        pro_right_indices = find(strcmp({stimulus_events.cond}, 'pro') & strcmp({stimulus_events.dir}, 'right'));
        anti_left_indices = find(strcmp({stimulus_events.cond}, 'anti') & strcmp({stimulus_events.dir}, 'left'));
        anti_right_indices = find(strcmp({stimulus_events.cond}, 'anti') & strcmp({stimulus_events.dir}, 'right'));
        ...
        %  RT information
        pro_left_rt = [stimulus_events(pro_left_indices).rt];
        pro_right_rt = [stimulus_events(pro_right_indices).rt];
        anti_left_rt = [stimulus_events(anti_left_indices).rt];
        anti_right_rt = [stimulus_events(anti_right_indices).rt];
        ...
        pro_left_avg = cellfun(@(x) average_trials(stimEEG, x), pro_left_binned, 'UniformOutput', false);
        pro_right_avg = cellfun(@(x) average_trials(stimEEG, x), pro_right_binned, 'UniformOutput', false);
        anti_left_avg = cellfun(@(x) average_trials(stimEEG, x), anti_left_binned, 'UniformOutput', false);
        anti_right_avg = cellfun(@(x) average_trials(stimEEG, x), anti_right_binned, 'UniformOutput', false);
        ...
        time = stimEEG.times;  % in ms
        chanlocs = stimEEG.chanlocs;
        
        results_folder = fullfile(subjectPath, 'dir_ext_info_ERP_stimlock_binned_results');
        if ~exist(results_folder, 'dir')
            mkdir(results_folder);
        end
        save(fullfile(results_folder, 'ext_info_ERP_stimlock_binned_results.mat'), ...
...
function avg_data = average_trials(stimEEG, trial_indices)
    avg_data = mean(stimEEG.data(:,:,trial_indices), 3);
end
