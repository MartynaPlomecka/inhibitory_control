...
cluster_times = {
    [-200 , -100, 0],    % Midfrontalale t
    [-250 , -80, 0 ],    % Centroparietal
    [-200 , -100, 0]     % Occipital
};
...
parfor sj = 1:numel(d)
    try
        ...
        pro_erp = mean(saccEEG.data(:, :, pro_indices), 3);
        anti_erp = mean(saccEEG.data(:, :, anti_indices), 3);
        
        results{sj} = struct('age_group', age_group, 'pro_erp', pro_erp, ...
            'anti_erp', anti_erp, 'times', saccEEG.times, 'chanlocs', saccEEG.chanlocs);
...
for t = 1:length(cluster_times{c})
    ...
    for row = 1:4
        for t = 1:length(cluster_times{c})
            ...
            % condition label only in the first column of each row
            if t == 1
